			...
			for i := 0; i <= length; i++ {
				if valX, okX := board[x + i]; okX {
					if valY, oky := valX[y]; oky {
						if valY != 0 && valY != lineNumber{
							valY = -1
							hits = append(hits,  math.Abs(float64(x + i)) + math.Abs(float64(y)))
						} else{
							valY = lineNumber
						}
					} else {
			...
			for i := 0; i <= length; i++ {
				if valX, okX := board[x]; okX {
					if valY, oky := valX[y + i]; oky {
						if valY != 0 && valY != lineNumber{
							valY = -1
							hits = append(hits,  math.Abs(float64(y + i)) + math.Abs(float64(x)))
						} else{
							valY = lineNumber
						}
					} else {
...
func findClosestHit()(distance float64){
	m := 9999999999.0
	for _, e := range hits {
		if e < m  && e > 0{
			m = e
